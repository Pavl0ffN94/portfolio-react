import ipAdressTrackerMin from './../img/ipAdressTracker_min.png';
import ipAdressTrackerBig from './../img/ipAdrdessTracker_big.png';

import costAccoMin from '../img/CostAcco_min.png';
import costAccoBig from '../img/CostAcco_big.png';

import guetarSchoolMin from '../img/GuetarSchool_min.png';
import guetarSchoolBig from '../img/GuetarSchool_big.png';

import jsQuizMin from '../img/jsQuiz_min.png';
import jsQuizBig from '../img/jsQuiz_big.png';

import restCountryMin from '../img/RestCountry_min.png';
import restCountryBig from '../img/RestCountry_big.png';

import dashboarAppMin from '../img/DashboardApp_min.png';
import dashboarAppBig from '../img/DashboardApp_big.png';

import searchGithubMin from '../img/SeachGithub_min.png';
import searchGithubBig from '../img/SeachGithub_big.png';

import ambulanceMin from '../img/Ambulance_min.png';
import ambulanceBig from '../img/Ambulance_big.png';

const projects = [
  {
    title: 'Guetar School',
    skills: ' HTML, CSS, Sass JavaScript, Git, PHP, Adaptive desing',
    img: guetarSchoolMin,
    imgBig: guetarSchoolBig,
    gitHabLink: 'https://github.com/Pavl0ffN94/guitar-tutor',
    webLink: 'https://pavl0ffn94.github.io/guitar-tutor/',
  },
  {
    title: 'Ip Adress Tracker',
    skills: ' HTML, CSS, JavaScript, Leaflet.js, IP Geolocation API, Fetch API',
    img: ipAdressTrackerMin,
    imgBig: ipAdressTrackerBig,
    gitHabLink:
      'https://github.com/Pavl0ffN94/js-practis/tree/master/ip-address-tracker-master',
    webLink: 'https://pavl0ffn-ipfinder.vercel.app/',
  },
  {
    title: 'Cost Accounting',
    skills: 'JavaScript, React, Redux Toolkit, Redux-persist, CSS-модули',
    img: costAccoMin,
    imgBig: costAccoBig,
    gitHabLink: 'https://github.com/Pavl0ffN94/cost-accounting',
    webLink: 'https://cost-accounting-6tvtxjwks-pavl0ffn94.vercel.app/login',
  },
  {
    title: 'JS Quiz',
    skills: ' HTML, CSS, JavaScript, JSON, LocalStorage, Git, GitHub Pages',
    img: jsQuizMin,
    imgBig: jsQuizBig,
    gitHabLink: 'https://github.com/Pavl0ffN94/JS-quiz',
    webLink: 'https://pavl0ffn94.github.io/JS-quiz/',
  },
  {
    title: 'Rest Country ',
    skills: ' React, Redux toolkit, RTK AsyncThunk, TypeScript',
    img: restCountryMin,
    imgBig: restCountryBig,
    gitHabLink: 'https://github.com/Pavl0ffN94/REST-Countri-api-RTK-TS',
    webLink: 'https://rest-countri-api-rtk-ts-pik2-iq82mx2vr-pavl0ffn94.vercel.app/',
  },
  {
    title: 'Dashboard App ',
    skills: ' React, Redux, Redux-persist, JavaScript',
    img: dashboarAppMin,
    imgBig: dashboarAppBig,
    gitHabLink: 'https://github.com/Pavl0ffN94/dashboard-app',
    webLink: 'https://dashboard-app-sepia.vercel.app/',
  },
  {
    title: 'Search Github',
    skills: ' React, Redux, TypeScript, SASS',
    img: searchGithubMin,
    imgBig: searchGithubBig,
    gitHabLink: 'https://github.com/Pavl0ffN94/TS-search-github',
    webLink: 'https://ts-search-github-jljtshm3s-pavl0ffn94.vercel.app/',
  },
  {
    title: 'Ambulance webite',
    skills: ' React,Vite,TypeScript, Adaptive desing, SASS, Framer Motion',
    img: ambulanceMin,
    imgBig: ambulanceBig,
    gitHabLink: 'https://github.com/Pavl0ffN94/Ambulance-website-refreshed',
    webLink: 'https://ambulance-website-refreshed-ahacm60ce-pavl0ffn94.vercel.app/',
  },
  
];

export {projects};
